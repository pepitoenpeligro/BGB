...
(function(window, undefined) {
					...
					default:
						if(key.length===1 || key===".com") {
							...
							if(key != ".com") {
								...
								key = (hasCaps.length>0 || hasCapsLock.length>0) ? key.toUpperCase() : key;
							}
	...
	function bindDropDown() {
		jQuery(controller.dropdownKey).on("mouseup", function(event, data) {
			...
			if(dragStart) {
				dragStart = false;
				var topPos = jQuery("#jim-case").hasClass("portrait") ? 88 : 66,
				comPos = topPos,
				childHeight = parseInt(jQuery(".options").height()) + parseInt(jQuery(".options").css('padding-top')),
				i=0;
				
				if(jQuery("#jim-case").hasClass("portrait") && parseInt(jQuery(controller.dropdownKey+" #jim-iphone-dd_options .dd_options").css('top'))*zoom>88) 
					topPos = 88;
				else if(jQuery("#jim-case").hasClass("landscape") && parseInt(jQuery(controller.dropdownKey+" #jim-iphone-dd_options .dd_options").css('top'))*zoom>66)
					topPos = 66;
				else {
					...
					switch(key) {
						...
						case "options":
							...
							if(jQuery("#jim-case").hasClass("portrait")) {
								...
								jQuery(controller.dropdownKey+" #jim-iphone-dd_options .dd_options").animate({ "top" : 68-($target.position().top*zoom) + "px", queue : false });
								jQuery(controller.dropdownKey+" #jim-iphone-dd_options_big .dd_options").animate({ "top" : 0-($target.position().top*zoom) + "px", queue : false });
							}
							break;
						case "dd_controls-done":
							var currentPos = parseInt(jQuery(controller.dropdownKey+" #jim-iphone-dd_options .dd_options").css('top')),
							topPos = jQuery("#jim-case").hasClass("portrait") ? 88 : 66,
							childHeight = parseInt(jQuery(".options").height()) + parseInt(jQuery(".options").css('padding-top')),
							$target;
			...
			if(dragStart) {
				...
				if(posY<offset.top) {
					...
					if(jQuery("#jim-case").hasClass("portrait")) {
						...
						jQuery(controller.dropdownKey+" #jim-iphone-dd_options .dd_options").animate({ "top" : -(childHeight*(ddSize-4))-1 +"px", queue:false});
						jQuery(controller.dropdownKey+" #jim-iphone-dd_options_big .dd_options").animate({ "top" : -(childHeight*(ddSize-4))-7-62 +"px", queue:false});
					}
					dragStart=false;
				}
					...
					if(jQuery("#jim-case").hasClass("portrait")) {
						...
						jQuery(controller.dropdownKey+" #jim-iphone-dd_options .dd_options").animate({ "top" : 66 + "px", queue:false });
						jQuery(controller.dropdownKey+" #jim-iphone-dd_options_big .dd_options").animate({ "top" : 0 + "px", queue:false });
					}
					dragStart=false;
				}
					...
					if(jQuery("#jim-case").hasClass("portrait")) {
						...
						if(jQuery(controller.dropdownKey+" #jim-iphone-dd_options").attr('id') === jQuery(event.target).parent().attr('id'))
							jQuery(controller.dropdownKey+" #jim-iphone-dd_options_big .dd_options").css("top", parseInt(jQuery(controller.dropdownKey+" #jim-iphone-dd_options .dd_options").css('top'))-66 +"px");
						else jQuery(controller.dropdownKey+" #jim-iphone-dd_options .dd_options").css("top", parseInt(jQuery(controller.dropdownKey+" #jim-iphone-dd_options_big .dd_options").css('top'))+66 +"px");
					}
				}
			}
	...
	var controller = {
		"keyboardKey":"#jim-ios11-kb",
		"dropdownKey":"#jim-ios11-dd",
		"dateKey":"#jim-ios11-da",
		"timeKey":"#jim-ios11-ti",
		"datetimeKey":"#jim-ios11-dt",
	...
	window.jimDevice.controllers["iphoneIOS11"] = controller;
